...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ColorCorrectionLookup : PostEffectsBase
    {
        ...
        public void SetIdentityLut () {
            int dim = 16;
            var newC = new UnityEngine.Color[dim * dim * dim];
            float oneOverDim = 1.0f / (1.0f * dim - 1.0f);

            for(int i = 0; i < dim; i++) {
                for(int j = 0; j < dim; j++) {
                    for(int k = 0; k < dim; k++) {
                        newC[i + (j * dim) + (k * dim * dim)] = new UnityEngine.Color((i * 1.0f)* oneOverDim, (j * 1.0f)* oneOverDim, (k * 1.0f)* oneOverDim, 1.0f);
                    }
                }
            }
        ...
        public void Convert ( Texture2D temp2DTex, string path) {
            ...
            if (temp2DTex) {
                ...
                var newC = new UnityEngine.Color[c.Length];
